...
public class DWGraph_Algo implements dw_graph_algorithms {
    ...
    private directed_weighted_graph revreseGraph() {
        dw_graph_algorithms graphAlgo = new DWGraph_Algo();
        graphAlgo.init(graph);
        directed_weighted_graph graph1 = graphAlgo.copy();
        for (node_data k : graph1.getV()) {
            for (edge_data m : graph1.getE(k.getKey())) {
                graph1.connect(m.getSrc(), k.getKey(), graph1.getEdge(k.getKey(), m.getSrc()).getWeight());
                graph1.removeEdge(k.getKey(), m.getSrc());
            }
        }
        return graph1;
    }
